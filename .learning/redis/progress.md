...
- 하네스 이력: 학습자 피드백으로 ① 쿼리수 assert(답지) 폐기 → SLO 계약, ② CLAUDE.md에 assert 규칙·자기검문·brief.md(§6-4) 추가, ③ JUnit 내장부하 폐기 → k6 전환(JUnit p99=408ms vs k6 173ms — 측정기 자체 잡음 2.3배 확인), ④ 학습자 요청으로 prometheus+grafana 관측 스택 추가(compose+`monitoring/` 코치 완성 제공, 앱 쪽 actuator 배선은 학습자가 직접). 초기 유출: "DB 접근 줄이기" 방향(후속 고통은 미유출).
- 최근 측정(2026-07-04, 학습자 재실행): p99=238ms, 1,078 req/s — 베이스라인(173ms/1,265)보다 악화. 코드 변경 여부 미확인, MySQL Questions 카운트 숙제 중.
...
- 컨테이너(고정): mysql:8 + redis:7(vanilla) + app(컨테이너) + nginx LB + prometheus/grafana(관측, localhost:3000). 평소 개발은 호스트 bootRun, 다중 고통만 `--scale app=N`.
